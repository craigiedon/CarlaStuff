...
from carlaUtils import set_sync, set_weather, delete_actors, setup_actors, create_cam, norm_salient_input, \
    ...
    model_detector, SnapshotEncoder
    ...
    try:
        ...
        for ep_id in range(num_episodes):
            ...
            for i in range(num_timesteps):
                ...
                # m_detection, m_centroid, m_depth = dummy_detector(salient_vars, other_v, ego_cam, world, 0.0)
            ...
            ego_cam.destroy()
            depth_cam.destroy()
            print(f"Ep time: {time.time() - start_time}")

            # Write the episode data to file
            with open(os.path.join(data_folder_name, f"e{ep_id}.json"), 'w') as fp:
                json.dump([dataclasses.asdict(s) for s in rollout_log], fp)
    finally:
        delete_actors(client, actor_list)
